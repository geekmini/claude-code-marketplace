...
if [ -n "$APP_ID" ]; then
  MESSAGE="mem0: Save session summary with add_memory (user_id=\"${USER_ID}\", app_id=\"${APP_ID}\" for project-specific)"
else
  MESSAGE="mem0: Save session summary with add_memory (user_id=\"${USER_ID}\")"
fi
